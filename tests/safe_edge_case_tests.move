...
fun test_set_batch_size_boundary_values() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    {
        ...
        safe::set_batch_size(&mut safe, &admin_cap, 1, ts::ctx(&mut scenario));
        assert!(safe::get_batch_size(&safe) == 1, 0);

        // Test maximum valid batch size (100)
        safe::set_batch_size(&mut safe, &admin_cap, 100, ts::ctx(&mut scenario));
...
fun test_batch_timeout_edge_cases() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    {
        ...
        safe::set_batch_timeout_ms(&mut safe, &admin_cap, 0, ts::ctx(&mut scenario));
        ...
        safe::set_batch_timeout_ms(&mut safe, &admin_cap, settle_timeout, ts::ctx(&mut scenario));
...
fun test_token_limit_updates() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    {
        ...
        safe::set_token_min_limit<TEST_COIN>(&mut safe, &admin_cap, 0, ts::ctx(&mut scenario));
        assert!(safe::get_token_min_limit<TEST_COIN>(&safe) == 0, 0);
...
fun test_bridge_address_updates() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    {
        ...
        safe::set_bridge_addr(&mut safe, &admin_cap, BRIDGE, ts::ctx(&mut scenario));
        assert!(safe::get_bridge_addr(&safe) == BRIDGE, 1);

        let new_bridge = @0xbeef;
        safe::set_bridge_addr(&mut safe, &admin_cap, new_bridge, ts::ctx(&mut scenario));
...
fun test_set_batch_size_wrong_admin() {
    ...
    ts::next_tx(&mut scenario, USER);
    {
        ...
        safe::set_batch_size(&mut safe, &admin_cap, 50, ts::ctx(&mut scenario));
